	...
	bot.WithDefaultHandler(func(ctx context.Context, b *bot.Bot, update *models.Update) {
	  ...
	  if update.Message != nil && update.Message.From != nil {
		...
		if strings.HasPrefix(text, "/config") || text == "/pending" || text == "/showdb" || text == "/cleardb" {
		  response := gosalebot.HandleAdminCommand(db, userID, text)
		...
		resp := gosalebot.HandleMessageWithDB(db, userID, text, b, update.Message.Chat.ID, update.Message.ID, photoFileIDs, ModerationGroupID, lang, username, strconv.Itoa(ModerationTopicID))
